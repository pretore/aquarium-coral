...
static void check_at_error_on_object_is_null(void **state) {
    coral_error = CORAL_ERROR_NONE;
    assert_false(coral_array_list_at(NULL, (void *) 1, (void *) 1));
    assert_int_equal(CORAL_ARRAY_LIST_ERROR_OBJECT_IS_NULL,
                     coral_error);
    coral_error = CORAL_ERROR_NONE;
}
...
static void check_at_error_on_item_is_null(void **state) {
    coral_error = CORAL_ERROR_NONE;
    assert_false(coral_array_list_at((void *) 1, NULL, (void *) 1));
    assert_int_equal(CORAL_ARRAY_LIST_ERROR_ITEM_IS_NULL,
                     coral_error);
    coral_error = CORAL_ERROR_NONE;
}
...
static void check_at_error_on_out_is_null(void **state) {
    coral_error = CORAL_ERROR_NONE;
    assert_false(coral_array_list_at((void *) 1, (void *) 1, NULL));
    assert_int_equal(CORAL_ARRAY_LIST_ERROR_OUT_IS_NULL,
                     coral_error);
    coral_error = CORAL_ERROR_NONE;
}
...
static void check_at(void **state) {
    ...
    assert_true(coral_array_list_add_all(&object, count, items));
    void *item;
    assert_true(coral_array_list_first(&object, &item));
    for (uintmax_t i = 0; i < count; i++) {
        uintmax_t at;
        assert_true(coral_array_list_at(&object, item, &at));
        assert_int_equal(at, i);
        if (1 + i < count) {
            assert_true(coral_array_list_next(&object, item, &item));
        }
    }
    assert_true(coral_array_list_invalidate(&object, NULL));
    coral_error = CORAL_ERROR_NONE;
...
static void check_at_error_on_item_is_out_of_bounds(void **state) {
    ...
    assert_true(coral_array_list_add_all(&object, count, items));
    uintmax_t at;
    assert_false(coral_array_list_at(&object, (void *) 1, &at));
    assert_int_equal(CORAL_ARRAY_LIST_ERROR_ITEM_IS_OUT_OF_BOUNDS,
                     coral_error);
    assert_true(coral_array_list_invalidate(&object, NULL));
    coral_error = CORAL_ERROR_NONE;
